...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run Umbrella's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./umbrella/tests"
    fi
  ...
  if [ setup.cfg -nt umbrella.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='umbrella/*' --omit='umbrella/openstack/common/*' -d covhtml -i
  fi
