...
from umbrella.common import log
...
looping_opts = [
    cfg.IntOpt('platform_periodic_fuzzy_delay',
               default=3600,
               help='range of seconds to randomly delay when starting the'
                    ' periodic task scheduler to reduce stampeding.'
                    ' (Disable by setting to 0)'),
    cfg.IntOpt('platform_periodic_interval',
               default=3600,
               help='seconds between running periodic tasks'),
    cfg.IntOpt('az_periodic_fuzzy_delay',
               default=3600),
    cfg.IntOpt('az_periodic_interval',
               default=3600),
    cfg.IntOpt('host_periodic_fuzzy_delay',
               default=3600),
    cfg.IntOpt('host_periodic_interval',
               default=3600),
    cfg.IntOpt('user_periodic_fuzzy_delay',
               default=60 * 60 * 12),
    cfg.IntOpt('user_periodic_interval',
               default=60 * 60 * 12),
    cfg.ListOpt('init_methods',
               default=['umbrella.openstack.api#init', ])
                ]
...
CONF.register_opts(looping_opts)
CONF.register_opt(time_opt)
...
class Server(object):
    ...
    def __init__(self, threads=1000):
        ...
        self.timers = []
        self.level_list = []
        self._register_level(models.PLATFORM_LEVEL,
                             CONF.platform_periodic_fuzzy_delay,
                             CONF.platform_periodic_interval)
        self._register_level(models.HOST_LEVEL,
                             CONF.host_periodic_fuzzy_delay,
                             CONF.host_periodic_interval)
        self._register_level(models.AZ_LEVEL,
                             CONF.az_periodic_fuzzy_delay,
                             CONF.az_periodic_interval)
        self._register_level(models.USER_LEVEL,
                             CONF.user_periodic_fuzzy_delay,
                             CONF.user_periodic_interval)
    ...
    def exec_init_method(self):
        for method_name in CONF.init_methods:
            module = importlib.import_module(method_name.split("#")[0])
            method = getattr(module, method_name.split("#")[1])
            method()

    def periodic_tasks(self, level):
        try:
            manager.Manager().periodic_tasks(level)
        except Exception:
            LOG.exception(_("Error occured when periodic task executes."))
    ...
    def _register_level(self, level, periodic_fuzzy_delay, periodic_interval):
        """
        register periodic tasks of different level
        """
        LOG.debug(_("Register periodic tasks for level %s"), level)
        level_dict = {
                       "level": level,
                       "periodic_fuzzy_delay": periodic_fuzzy_delay,
                       "periodic_interval": periodic_interval
                      }
        self.level_list.append(level_dict)

    def start_looping(self):
        '''
        start looping call for pushing data to monitor
        and for checking alarming
        '''
        for level in self.level_list:
            if level["periodic_fuzzy_delay"] == -1 or \
                    level["periodic_interval"] == -1:
                continue
            initial_delay = random.randint(0, level["periodic_fuzzy_delay"])
            periodic = utils.LoopingCall(self.periodic_tasks,
                                         level=level["level"])
            periodic.start(interval=level["periodic_interval"],
                           initial_delay=initial_delay)
            LOG.info(_("periodic tasks for level %s started."), level)
            self.timers.append(periodic)

    def wait_looping(self):
        for x in self.timers:
            try:
                x.wait()
            except Exception, e:
                LOG.info(_("wait looping call exception: %s") % str(e))

    def stop_loopings(self):
        '''
        stop all looping call
        '''
        for x in self.timers:
            try:
                x.stop()
    ...
    def run_looping_in_child(self):
        '''
        run looping call in child process
        '''
        pid = os.fork()
        if pid == 0:
            ...
            self.start_looping()
            self.wait_looping()
            LOG.info(_('Child(for looping call) %d exiting normally') %
                     os.getpid())
            sys.exit(0)
        else:
            LOG.info(_('Started child(for looping call) %s') % pid)
...
class JSONResponseSerializer(object):
    ...
    def to_json(self, data):
        def sanitizer(obj):
            if isinstance(obj, datetime.datetime) and CONF.use_local_time:
                obj = timeutils.utc_to_local(obj)
                return timeutils.strtime(obj, "%Y-%m-%d %H:%M:%S")
