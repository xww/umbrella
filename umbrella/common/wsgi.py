...
task_opts = [
    cfg.ListOpt("monitor_items", default = ["cpu", "disk", "net", "mem"],
...
class Server(object):
    ...
    def start(self, application, default_port):
        ...
        self.configure()
        self.start_wsgi()
        LOG.info(_LI("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"))
    ...
    def start_wsgi(self):
        if CONF.workers == 0:
            ...
            if self.default_port == 9801:
                for item in cfg.CONF.monitor_items:
                    self.pool.spawn_n(
                        self.__getattribute__("start_looping_%s" % item))
            return
        else:
            if self.default_port == 9801:
                #(xww):add support multi workers for umbrella-collector later
                LOG.info(_LI("At current time only support workers = 0"))
                return
    ...
    def start_looping_cpu(self):

        def periodic_task_cpu():
            try:
                cpu.CpuController().collect_cpu_info()
            except Exception:
                 LOG.exception(_LI("Error occured when disk"
                                   " periodic task executes."))
        cpu_periodic = utils.LoopingCall(periodic_task_cpu)
        cpu_periodic.start(interval=cfg.CONF.cpu_task_period,
                            initial_delay=3)
        self.timers.append(cpu_periodic)


    def start_looping_disk(self):
        def periodic_task_disk():
            try:
                disk.DiskController().collect_disk_info()
            except Exception:
                LOG.exception(_LI("Error occured when disk"
                                  " periodic task executes."))

        disk_periodic = utils.LoopingCall(periodic_task_disk)
        disk_periodic.start(interval=cfg.CONF.disk_task_period,
                            initial_delay=3)
        self.timers.append(disk_periodic)

    def start_looping_net(self):
        def periodic_task_net():
            try:
                net.NetController().collect_net_info()
            except Exception:
    ...
    def start_looping_mem(self):
        def periodic_task_mem():
            try:
                mem.MemController().collect_mem_info()
            except Exception:
                LOG.exelf.pooleption(_LI("Error occured when mem"
                                  " periodic task executes."))

        mem_periodic = utils.LoopingCall(periodic_task_mem)
        mem_periodic.start(interval=cfg.CONF.mem_task_period, initial_delay=3)
        self.timers.append(mem_periodic)
...
class Request(webob.Request):
    ...
    def best_match_language(self):
        ...
        if not self.accept_language:
            return None
        langs = i18n.get_available_languages('umbrella')
