...
from umbrella.common import auth
...
class BaseClient(object):
    ...
    def do_request(self, method, action, body=None, headers=None,
        ...
        """
        if should_auth and not self.auth_tok:
            self._authenticate()
    ...
    def _do_request(self, method, url, body, headers):
        ...
        try:
            ...
            if status_code in self.OK_RESPONSE_CODES:
                ...
                raise webob.exc.HTTPRedirection(res.getheader('Location'))
            elif status_code == httplib.UNAUTHORIZED:
                raise webob.exc.HTTPUnauthorized(res.read())
            elif status_code == httplib.FORBIDDEN:
                raise webob.exc.HTTPForbidden(res.read())
            elif status_code == httplib.NOT_FOUND:
                raise webob.exc.HTTPNotFound(res.read())
            elif status_code == httplib.CONFLICT:
                raise webob.exc.HTTPConflict(res.read())
            elif status_code == httplib.BAD_REQUEST:
                raise webob.exc.HTTPClientError(body=res.read())
            elif status_code == httplib.MULTIPLE_CHOICES:
                raise webob.exc.HTTPMultipleChoices(body=res.read())
            elif status_code == httplib.REQUEST_ENTITY_TOO_LARGE:
                raise webob.exc.HTTPRequestEntityTooLarge(retry=_retry(res),
                                              body=res.read())
            elif status_code == httplib.INTERNAL_SERVER_ERROR:
                raise webob.exc.HTTPServerError()
            elif status_code == httplib.SERVICE_UNAVAILABLE:
                raise webob.exc.HTTPServiceUnavailable(retry=_retry(res))
            else:
