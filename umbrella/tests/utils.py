...
def minimal_add_command(port, name, suffix='', public=True):
    ...
    return ("bin/umbrella --port=%d add %s"
...
class FakeDB(object):
    ...
    def dict_to_setting_ref(self, dict_data):
        setting_ref = models.Settings()
        setting_ref.update(dict_data)
        return setting_ref

    def setting_set(self, level, setting_type, capacity=0, threshold=0.0,
                    alarm_title=None, alarm_content=None, deleted=False,
                    enable=True):
        setting = dict({"uuid": utils.generate_uuid(),
                        "enable": enable,
                        "deleted": deleted,
                        "level": level,
                        "type": setting_type,
        ...
        self.setting.update({(level, setting_type): setting})

        return self.dict_to_setting_ref(setting)

    def setting_get_by_lever_type(self, level, setting_type, session=None):
        result = self.setting.get((level, setting_type), None)
        if result is None:
            raise exception.NotFound()
        return self.dict_to_setting_ref(result)

    def alarming_create(self, values, session=None):
        id = utils.generate_uuid()
        values.update(id=id)
        self.alarms.update({id: values})
        return values

    def alarming_get_all(self):
        return self.alarms

    def alarming_get(self, id):
        result = self.alarms.get(id, None)
        if result is None:
            raise exception.NotFound()
        return result

    def alarming_clear(self):
        self.alarms = {}
